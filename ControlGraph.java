...
class CondTransition extends TransitionLabel {
    ...
    NatureTransition getNature() {
        return NatureTransition.COND;
    }
...
class AssignTransition extends TransitionLabel {
    ...
    NatureTransition getNature() {
        return NatureTransition.ASSIGN;
    }

    AssignTransition(AffNode a) {
        theAssign = a;
    }
...
class ControlGraph {
    ...
    public int getNumberOfIntIdfs() {
        return numberOfIntIdfs;
    }
    ...
    void addLast(ControlGraph g) {
        ...
        exit.theTransitions.addAll(g.entry.theTransitions);
        theControlPoints.remove(g.entry);
        g.theControlPoints.forEach(cp->{
            cp.theTransitions.forEach(cpt -> {
                if (cpt.theTarget == g.entry){
                    cpt.theTarget = exit;
                }
            });
        });
    ...
    ControlGraph(ExprNode cond, ControlGraph gthen, ControlGraph gelse) {
        ...
        if (gelse != null)
            this.generateCondition(cond, entry, gthen.entry, gelse.entry, theControlPoints);
        else
            this.generateCondition(cond, entry, gthen.entry, exit, theControlPoints);
    ...
    ControlGraph(ExprNode cond, ControlGraph gbody) {
        ...
        this.generateCondition(cond, entry, gbody.entry, exit, theControlPoints);
    ...
    (ExprNode ec,
     ...
     List<ControlPoint> cp) {
        if (ec.theKind == ExprNode.Kind.BINARY || ec.theKind == ExprNode.Kind.UNARY) {
            if (ec.operator.theOp == OperatorKind.AND) {

                ControlPoint aux = new ControlPoint("and_rhs");
                cp.add(aux);

                generateCondition(ec.fg, begin, aux, ff, cp);
                generateCondition(ec.fd, aux, tt, ff, cp);

            } else if (ec.operator.theOp == OperatorKind.OR) {
                ControlPoint aux = new ControlPoint("or_rhs");

                cp.add(aux);
                generateCondition(ec.fg, begin, tt, aux, cp);
                generateCondition(ec.fd, aux, tt, ff, cp);

            } else if (ec.operator.theOp == OperatorKind.NOT) {
                generateCondition(ec.fg, begin, ff, tt, cp);
            } else {
